...
@then(parsers.parse('the new group list is equal to the old list with the group'), target_fixture="verify_group_added")
...
def non_empty_group_list(db, app):
    if len(db.get_group_list()) == 0:
        app.group.create(Group(name='name1'))
    return db.get_group_list()
...
@when(parsers.parse('i delete the group from the list'), target_fixture="del_group")
def del_group(app, random_group):
    app.group.delete_group_by_id(random_group.id)

@then(parsers.parse('the new group list is equal to the old list without deleted group'), target_fixture="verify_group_deleted")
